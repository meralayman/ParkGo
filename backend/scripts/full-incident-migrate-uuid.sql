-- Run ONCE as PostgreSQL superuser (e.g. postgres), not as parkgo_user:
--   psql -U postgres -d parkgo_db -f backend/scripts/full-incident-migrate-uuid.sql
-- Or paste into pgAdmin Query Tool connected as postgres.
...
-- Lets the app user run ALTERs from ensureIncidentReportsTable(); change name if DATABASE_URL uses another role.
